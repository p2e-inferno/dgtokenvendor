...
export const menuLinks: HeaderMenuLink[] = [
  ...
  },
  {
    label: "Token Vendor",
    href: "/token-vendor",
    icon: <CurrencyDollarIcon className="h-4 w-4 text-primary" />,
  },
  {
    label: "Profile",
    href: "/profile",
    icon: <UserCircleIcon className="h-4 w-4 text-accent" />,
  },
  {
    label: "Power Up",
    href: "/power-up",
    icon: <FireIcon className="h-4 w-4 text-secondary" />,
  },
  {
    label: "Events",
    href: "/events",
    icon: <BoltIcon className="h-4 w-4 text-info" />,
  },
];
        ...
        return (
          <li key={href}>
            <Link
              ...
              } hover:bg-secondary hover:shadow-md focus:!bg-secondary active:!text-neutral py-2 px-4 text-sm rounded-xl gap-2 grid grid-flow-col transition-all duration-200`}
            >
              {icon}
              <span>{label}</span>
            </Link>
